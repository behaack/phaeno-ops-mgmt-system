...
- The HTML landing page is the single internal Website search result and the
  ...
  artifact.
- The Website implements the Phase 1 publication convention, metadata,
  structured data, and configured PDF indexing policy. The backend crawler
  implements guarded publication-PDF enrichment locally; production behavior
  still depends on an authorized deployment and completed index rebuild.
...
- PdfPig 0.1.15 is the approved and implemented .NET extraction dependency.
...
- A result matched only by PDF body text still opens the landing page. Its
  ...
  reliably honor. The Website labels any result that requires source text as
  `Match in linked PDF` so the destination is not misleading.
...
- `backend/app/Features/Website/Crawler/WebsiteCrawler.cs` discovers sitemap
  HTML URLs, extracts visible HTML headings and sections, and performs guarded,
  bounded enrichment from declared first-party PDF sources.
...
- The backend uses PdfPig 0.1.15 behind
  `IWebsiteDocumentTextExtractor` for approved first-party PDF sources.
...
themselves to return a result. For ordinary pages, hidden search titles and
summaries follow the same candidate-only rule: every term must also occur in
the visible destination heading or section text. Approved PDF source text is
the only indexed-content exception.
...
- no ordinary-page result supported only by a hidden result title or summary;
- all query terms supported by visible destination HTML or approved PDF text;
- a source-dependency flag for a result that requires PDF text;
...
- Search for a PDF-only term returns the same landing result with a supported
  snippet and a visible `Match in linked PDF` label.
- Search for a term present only in an ordinary page's hidden search title,
  summary, or keywords returns no result.
