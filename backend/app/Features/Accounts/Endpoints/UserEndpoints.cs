        ...
        CancellationToken cancellationToken)
    {
        ...
        if (requestedBusinessRoles.Count > 0)
        {
            foreach (var role in allBusinessRoles)
            {
                ...
                if (assignment?.Version != requestedRole.Version) throw new DbUpdateConcurrencyException();
                if (requestedRole.IsActive
                    && user is not { IsActive: true, Status: UserAccountStatus.Active })
                    throw new BadRequestException("Business roles can be assigned only to active Phaeno users.");
                if (assignment == null)
                {
                ...
                else assignment.SetActive(requestedRole.IsActive);
            }
        }
