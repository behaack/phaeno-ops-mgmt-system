...
public sealed class CrmWorkController(PSeqOperationsDbContext dbContext, IExternalIdentityContext externalIdentityContext) : ControllerBase
{
    ...
    public async Task<IReadOnlyList<CrmSearchResultDto>> Search([FromQuery] string query, CancellationToken cancellationToken)
    {
        ...
        results.AddRange(contacts.Select(value => new CrmSearchResultDto(CrmRecordType.Contact, value.Id, value.Title, value.Email,
            value.CommunicationPreference is CrmCommunicationPreference.OptedOut or CrmCommunicationPreference.DoNotContact or CrmCommunicationPreference.Suppressed ? "Outreach suppressed" : "Review outreach decision", value.UpdatedAt)));
