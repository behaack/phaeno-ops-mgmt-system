...
export type LabServiceOrderingEligibility = {
...
export type ManualJournalEntryRow = {
  documentId: string;
  entryId: string;
  ...
  customerOrProjectReference: string | null;
  purchaseOrderNumber: string | null;
  ...
  paymentRecordedAtUtc: string | null;
  memo: string;
  version: number;
};
...
export async function submitLabOrder(id: string, version: number) {
  return post<LabServiceOrder>(
    `/lab-service-orders/${id}/submit-for-quote`,
    { version },
    true,
  );
}
export async function withdrawLabOrder(
  id: string,
  version: number,
  reason: string,
) {
  return post<LabServiceOrder>(`/lab-service-orders/${id}/withdraw`, {
    version,
    reason,
  });
}
...
export async function addLabSample(
  orderId: string,
  input: LabSampleRosterWrite,
) {
  return post<LabServiceOrder>(`/lab-service-orders/${orderId}/samples`, input);
}
export async function updateLabSample(
  orderId: string,
  sampleId: string,
  input: LabSampleRosterWrite,
) {
  return patch<LabServiceOrder>(
    `/lab-service-orders/${orderId}/samples/${sampleId}`,
    input,
  );
}
export async function deleteLabSample(
  orderId: string,
  sampleId: string,
  version: number,
) {
  const response = await api.delete<ApiEnvelope<LabServiceOrder>>(
    `/lab-service-orders/${orderId}/samples/${sampleId}`,
    { data: { version } },
  );
...
export async function previewLabSampleImport(
  orderId: string,
  file: File,
  version: number,
) {
  ...
  const response = await api.post<ApiEnvelope<LabSampleImportPreview>>(
    `/lab-service-orders/${orderId}/samples/import-preview`,
    form,
    { headers: { "Content-Type": undefined } },
  );
  return unwrap(response.data);
}
export async function confirmLabSampleImport(
  orderId: string,
  previewId: string,
  version: number,
) {
  return post<LabServiceOrder>(
    `/lab-service-orders/${orderId}/samples/import-previews/${previewId}/confirm`,
    { version },
  );
}
...
export async function downloadLabSampleTemplate(
  orderId: string,
  orderNumber: string,
) {
  const response = await api.get<Blob>(
    `/lab-service-orders/${orderId}/samples/template.csv`,
    { responseType: "blob" },
  );
  saveBlob(response.data, `${orderNumber}-sample-list.csv`);
}
export async function requestLabCancellation(
  id: string,
  version: number,
  reason: string,
) {
  return post<LabServiceOrder>(
    `/lab-service-orders/${id}/cancellation-requests`,
    { version, reason, scopeJson: "{}" },
    true,
  );
}
export async function recordLabSampleShipment(
  orderId: string,
  sampleId: string,
  input: {
    version: number;
  ...
  return api
    .put<
      ApiEnvelope<LabServiceOrder>
    >(`/lab-service-orders/${orderId}/samples/${sampleId}/shipment`, input)
    .then((response) => unwrap(response.data));
}
export async function downloadLabResult(
  orderId: string,
  file: OperationalFile,
) {
  const response = await api.get<Blob>(
    `/lab-service-orders/${orderId}/results/${file.id}/download`,
    { responseType: "blob" },
  );
  saveBlob(response.data, file.fileName);
}
export async function downloadLabResultPackage(
  orderId: string,
  releaseId: string,
  releaseVersion: number,
) {
  const response = await api.get<Blob>(
    `/lab-service-orders/${orderId}/results/releases/${releaseId}/download`,
    { responseType: "blob" },
  );
  saveBlob(response.data, `results-release-${releaseVersion}.zip`);
}

export async function listReagentOrders(
  params?: Record<string, string | number | undefined>,
) {
  return get<PagedResult<OrderListItem>>("/reagent-orders", params);
}
export async function getReagentOrder(id: string) {
  return get<ReagentOrder>(`/reagent-orders/${id}`);
}
export async function listReagentOfferings() {
  return get<ReagentOffering[]>("/order-catalog/reagent-offerings");
}
export async function listShippingAddresses() {
  return get<ShippingAddress[]>("/partner-shipping-addresses");
}
export async function createShippingAddress(
  input: Omit<ShippingAddress, "id" | "isActive" | "version">,
) {
...
export async function updateReagentOrder(
  id: string,
  lines: Array<{ offeringId: string; quantity: number; note?: string }>,
  version: number,
) {
...
export async function decideReagentAdjustment(
  ...
  return post<ReagentOrder>(
    `/reagent-orders/${orderId}/adjustments/${adjustmentId}/decision`,
    { version, approved },
  );
}
export async function requestReagentCancellation(
  id: string,
  version: number,
  reason: string,
) {
  return post<ReagentOrder>(
    `/reagent-orders/${id}/cancellation-requests`,
    { version, reason, scopeJson: "{}" },
    true,
  );
}
export async function cancelReagentOrder(
  id: string,
  version: number,
  reason: string,
) {
  return post<ReagentOrder>(`/reagent-orders/${id}/cancel`, {
    version,
    reason,
  });
}

export async function listAssemblyRequests(
  params?: Record<string, string | number | undefined>,
) {
  return get<PagedResult<OrderListItem>>("/data-assembly-requests", params);
}
export async function getAssemblyRequest(id: string) {
  return get<DataAssemblyRequest>(`/data-assembly-requests/${id}`);
}
export async function listAssemblyProfiles() {
  return get<AssemblyProfile[]>("/order-catalog/assembly-profiles");
}
export async function createAssemblyRequest(input: {
  assemblyProfileId: string;
  projectReference: string;
  metadataJson: string;
  requestedOutput: string;
...
export async function submitAssemblyRequest(
  id: string,
  version: number,
  files: OperationalFile[],
) {
  return post<DataAssemblyRequest>(
    `/data-assembly-requests/${id}/submit`,
    {
      version,
...
export async function acceptAssemblyQuote(
  id: string,
  quoteId: string,
  version: number,
  purchaseOrderNumber: string,
  ...
  return post<DataAssemblyRequest>(
    `/data-assembly-requests/${id}/quotes/${quoteId}/accept`,
    { version, quoteId, purchaseOrderNumber },
    true,
  );
}
export async function requestAssemblyCancellation(
  id: string,
  version: number,
  reason: string,
) {
  return post<DataAssemblyRequest>(
    `/data-assembly-requests/${id}/cancellation-requests`,
    { version, reason, scopeJson: "{}" },
    true,
  );
}
export async function withdrawAssemblyRequest(
  id: string,
  version: number,
  reason: string,
) {
  return post<DataAssemblyRequest>(`/data-assembly-requests/${id}/withdraw`, {
    version,
    reason,
  });
}
export async function downloadAssemblyOutput(
  requestId: string,
  releaseId: string,
  file: OperationalFile,
) {
  const response = await api.get<Blob>(
    `/data-assembly-requests/${requestId}/outputs/${releaseId}/files/${file.id}/download`,
    { responseType: "blob" },
  );
  saveBlob(response.data, file.fileName);
}
export async function downloadAssemblyOutputPackage(
  requestId: string,
  releaseId: string,
  releaseVersion: number,
) {
  const response = await api.get<Blob>(
    `/data-assembly-requests/${requestId}/outputs/${releaseId}/download`,
    { responseType: "blob" },
  );
...
export async function exportOrderList(
  ...
  const response = await api.get<Blob>(`/${path}/export`, {
    params,
    responseType: "blob",
  });
  saveBlob(
    response.data,
    `${path}-${new Date().toISOString().slice(0, 10)}.csv`,
  );
}
...
export async function updateOperationalAssignment(
  workflow: "lab" | "reagent" | "assembly",
  id: string,
  input: { version: number; assignToMe: boolean; dueAt?: string | null },
) {
  return api
    .put<
      ApiEnvelope<{
    ...
    >(`/platform/order-assignments/${workflow}/${id}`, input)
    .then((response) => unwrap(response.data));
}
  ...
  const path =
    workflow === "lab"
      ...
      : workflow === "reagent"
        ...
        : "data-assembly-requests";
  if (workflow === "lab")
    return get<LabServiceOrder>(`/platform/${path}/${id}`);
  if (workflow === "reagent")
    return get<ReagentOrder>(`/platform/${path}/${id}`);
  return get<DataAssemblyRequest>(`/platform/${path}/${id}`);
}
export type LabIntake = {
  orderId: string;
  orderNumber: string;
  workOrderId: string;
};
...
export async function runPlatformAction<T>(
  path: string,
  body: unknown,
  idempotent = false,
) {
...
export async function issuePlatformQuote(
  ...
  const path =
    workflow === "lab" ? "lab-service-orders" : "data-assembly-requests";
  return runPlatformAction<LabServiceOrder | DataAssemblyRequest>(
    `${path}/${id}/quotes`,
    input,
    true,
  );
}
export async function receivePlatformLabSample(
  orderId: string,
  sampleId: string,
  input: { version: number; receivedAt: string; receiptCondition: string },
) {
  return runPlatformAction<LabServiceOrder>(
    `lab-service-orders/${orderId}/samples/${sampleId}/receive`,
    input,
  );
}
export async function accessionPlatformLabSample(
  orderId: string,
  sampleId: string,
  input: { version: number; accessionId: string },
) {
  return runPlatformAction<LabServiceOrder>(
    `lab-service-orders/${orderId}/samples/${sampleId}/accession`,
    input,
  );
}
export async function transitionPlatformLabSample(
  orderId: string,
  sampleId: string,
  input: {
    version: number;
  ...
  return runPlatformAction<LabServiceOrder>(
    `lab-service-orders/${orderId}/samples/${sampleId}/transition`,
    input,
  );
}
export async function uploadPlatformLabResult(
  orderId: string,
  sampleId: string,
  input: {
    file: File;
...
export async function releasePlatformLabResult(
  orderId: string,
  sampleId: string,
  releaseId: string,
  version: number,
  ...
  return runPlatformAction<LabServiceOrder>(
    `lab-service-orders/${orderId}/samples/${sampleId}/results/${releaseId}/release`,
    { version },
    true,
  );
}
...
export async function uploadPlatformAssemblyOutput(
  requestId: string,
  runId: string,
  file: File,
) {
  ...
  form.append("file", file);
  const response = await api.post<ApiEnvelope<OperationalFile>>(
    `/platform/data-assembly-requests/${requestId}/processing-runs/${runId}/outputs`,
    form,
    { headers: { "Content-Type": undefined } },
  );
  return unwrap(response.data);
}
export async function getOrderConfiguration() {
...
export async function saveCatalogItem(
  ...
  return id
    ? patch<OrderConfiguration["catalogItems"][number]>(
        `/platform/order-configuration/catalog/items/${id}`,
        input,
      )
    : post<OrderConfiguration["catalogItems"][number]>(
...
export async function saveAnalysisDefinition(
  id: string | null,
  input: Omit<AnalysisDefinition, "id">,
) {
  return id
    ? patch<AnalysisDefinition>(
        `/platform/order-configuration/analyses/${id}`,
        input,
      )
    : post<AnalysisDefinition>("/platform/order-configuration/analyses", input);
}
export async function saveReagentOffering(
  ...
  return id
    ? patch<ReagentOffering>(
        `/platform/order-configuration/reagent-offerings/${id}`,
        input,
      )
    : post<ReagentOffering>(
        "/platform/order-configuration/reagent-offerings",
        input,
      );
}
export async function saveAssemblyProfile(
  id: string | null,
  input: Omit<AssemblyProfile, "id">,
) {
  return id
    ? patch<AssemblyProfile>(
        `/platform/order-configuration/assembly-profiles/${id}`,
        input,
      )
    : post<AssemblyProfile>(
        "/platform/order-configuration/assembly-profiles",
        input,
      );
}
export async function saveCommercialProfile(input: {
  organizationId: string;
  labCreditApproved: boolean;
  assemblyCreditApproved: boolean;
  qboCustomerId: string | null;
  version?: number;
}) {
  return api
    .put<
      ApiEnvelope<OrderConfiguration["commercialProfiles"][number]>
    >(`/platform/order-configuration/commercial-profiles/${input.organizationId}`, input)
    .then((response) => unwrap(response.data));
}
export async function listNotificationMessages(status?: string) {
  return get<PagedResult<NotificationMessage>>(
    "/platform/order-notifications",
    { status },
  );
}
export async function listIntegrationMessages(status?: string) {
  return get<PagedResult<IntegrationMessage>>("/platform/order-integrations", { status });
}
export async function retryIntegrationMessage(id: string, version: number) {
  return post<IntegrationMessage>(`/platform/order-integrations/${id}/retry`, { version }, true);
}
export async function retryNotificationMessage(id: string, version: number) {
  return post<NotificationMessage>(
    `/platform/order-notifications/${id}/retry`,
    { version },
    true,
  );
}
export async function listManualJournalEntries(from: string, to: string) {
  return get<ManualJournalEntryRow[]>(
    "/platform/order-accounting/journal-entries",
    { from, to },
  );
}
export async function downloadManualJournalEntries(from: string, to: string) {
  const response = await api.get<Blob>(
    "/platform/order-accounting/journal-entries.csv",
    { params: { from, to }, responseType: "blob" },
  );
  saveBlob(response.data, `journal-entry-source-${from}-${to}.csv`);
}
