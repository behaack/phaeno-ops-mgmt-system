...
export const trialLabel = (value: string) => value.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/_/g, ' ').replace(/^./, letter => letter.toUpperCase())
...
export function requiredTrialInputs(scope: TrialScope | null) {
  ...
  return allRequiredTrialInputs(scope).filter(name => !builtIn.has(name.toLowerCase()))
}
export function allRequiredTrialInputs(scope: TrialScope | null) {
  ...
  for (const analysis of scope?.analyses ?? []) {
    ...
    for (const entry of values) {
      ...
      if (name) fields.set(name.toLowerCase(), name)
    }
  }
