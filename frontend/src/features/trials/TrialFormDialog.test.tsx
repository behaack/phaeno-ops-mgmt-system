...
describe('Trial bounded actions', () => {
  it('validates required fields on blur and clears their existing error while correcting them', async () => {
    ...
    const estimate = screen.getByLabelText('Estimate', { exact: false })
  ...
  it('preserves entered values while explicitly reloading a concurrency conflict', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Estimate', { exact: false }), { target: { value: 'Next week' } }); fireEvent.click(screen.getByRole('button', { name: 'Save' }))
    await screen.findByRole('alert'); fireEvent.click(screen.getByRole('button', { name: 'Reload current Trial; keep my entries' }))
    await screen.findByRole('status'); expect((screen.getByLabelText('Estimate', { exact: false }) as HTMLInputElement).value).toBe('Next week')
