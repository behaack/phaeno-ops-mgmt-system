...
describe('Trial sample roster', () => {
  ...
  it('locks the roster during reload, preserves it on failure and renews confirmation after changed-scope recovery', async () => {
    ...
    for (const [label, value] of Object.entries({ 'Coded sample reference': 'RNA-1', 'Biological source': 'Research RNA', 'Quantity (ng)': '100', 'Storage requirements': 'Frozen', 'Research material safety declaration': 'Nonhazardous', Organism: 'Synthetic organism' })) fireEvent.change(screen.getByLabelText(label, { exact: false }), { target: { value } })
    ...
    expect(screen.getByLabelText('Coded sample reference', { exact: false }).matches(':disabled')).toBe(true); expect(screen.getByRole('checkbox').matches(':disabled')).toBe(true)
    ...
    expect((screen.getByLabelText('Coded sample reference', { exact: false }) as HTMLInputElement).value).toBe('RNA-1'); expect((screen.getByRole('checkbox') as HTMLInputElement).checked).toBe(true)
    ...
    expect((screen.getByLabelText('Coded sample reference', { exact: false }) as HTMLInputElement).value).toBe('RNA-1'); expect((screen.getByRole('checkbox') as HTMLInputElement).checked).toBe(false)
    fireEvent.click(screen.getByRole('checkbox')); fireEvent.click(screen.getByRole('button', { name: 'Submit 1 sample' }))
