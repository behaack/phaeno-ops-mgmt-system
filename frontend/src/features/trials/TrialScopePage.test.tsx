...
describe('Trial scope drafts', () => {
  it('saves incomplete work separately while approval submission requires complete scope', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Trial name', { exact: false }), { target: { value: 'Early research idea' } })
  ...
  it('protects both actions and navigation during a draft save', async () => {
    ...
    render(<TrialScopePage trialId={trial.id} />)
    fireEvent.change(screen.getByLabelText('Trial name', { exact: false }), { target: { value: 'Draft for tomorrow' } })
    ...
    expect(screen.getByLabelText('Trial name', { exact: false }).matches(':disabled')).toBe(true)
...
describe('Trial scope reload', () => {
  it('prevents edits, submission and navigation while reloading, then permits retry with the refreshed version', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Reason for this scope revision', { exact: false }), { target: { value: 'Keep the research goal' } })
    ...
    expect(screen.getByLabelText('Trial name', { exact: false }).matches(':disabled')).toBe(true); expect(screen.getByLabelText('Reason for this scope revision', { exact: false }).matches(':disabled')).toBe(true)
    expect(screen.getByRole('button', { name: 'Submit scope for approval' }).matches(':disabled')).toBe(true)
    ...
    await act(async () => { fireEvent.submit(screen.getByLabelText('Trial name', { exact: false }).closest('form')!) })
    ...
    expect((screen.getByLabelText('Reason for this scope revision', { exact: false }) as HTMLInputElement).value).toBe('Keep the research goal'); expect(screen.getByLabelText('Trial name', { exact: false }).matches(':disabled')).toBe(false)
