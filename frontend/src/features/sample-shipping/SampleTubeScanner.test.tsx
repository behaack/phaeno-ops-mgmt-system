...
describe('guided tube scanning', () => {
  ...
  it('saves before advancing, shows the exact permanent barcode and focuses the next tube', async () => {
    ...
    expect(input.closest('li')?.getAttribute('aria-current')).toBe('step')
    ...
    expect(screen.getByText('Tube barcode: TUBE_0001')).toBeTruthy()
    expect(screen.queryByRole('img', { name: 'Tube barcode TUBE_0001' })).toBeNull()
  ...
  it('reviews another page without changing the scan target, then advances past the eighth tube only after saving', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Return to active tube' })).toBeTruthy()
    ...
    expect(within(nextInput.closest('li')!).getByText(/Tube 9 of 18/)).toBeTruthy()
  ...
  it('retains a failed inline barcode while browsing and returning to the active tube', async () => {
    mocks.assign.mockRejectedValue(new Error('This tube belongs to another container.'))
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Return to active tube' }))
    const resumedInput = screen.getByLabelText(/Scan tube barcode/)
    expect(resumedInput).toHaveProperty('value', 'WRONG-TUBE')
    await waitFor(() => expect(document.activeElement).toBe(resumedInput))
    expect(screen.getByText('Tubes 1–8 of 10 in this container')).toBeTruthy()
    expect(within(resumedInput.closest('li')!).getByText('Tube was not matched')).toBeTruthy()
  ...
  it('keeps a Member view read-only and shows saved barcodes', () => {
    ...
    expect(screen.getByText('Tube barcode: SAVED-0001')).toBeTruthy()
    expect(screen.queryByRole('img', { name: 'Tube barcode SAVED-0001' })).toBeNull()
  })
})
