...
import { requestStatusLabel, serviceLabel } from './OrganizationListPage'
...
export function OrganizationDetailPage({ organizationId }: { organizationId: string }) {
  ...
  const orderToCashFeaturesQuery = useQuery({ queryKey: ['order-to-cash-features'], queryFn: getOrderToCashFeatures })
  const readinessQuery = useQuery({
    queryKey: ['operational-readiness', organizationId],
    queryFn: () => getOperationalReadiness(organizationId),
    enabled: organization?.kind === 'Customer' && orderToCashFeaturesQuery.data?.derivedReadiness === true,
  })
  ...
  const error = organizationQuery.error ?? summaryQuery.error ?? usersQuery.error ?? invitationsQuery.error ?? entitlementsQuery.error ?? requestsQuery.error ?? orderToCashFeaturesQuery.error ?? readinessQuery.error ?? editMutation.error ?? conversionMutation.error ?? inviteMutation.error ?? memberMutation.error ?? inviteAction.error ?? developmentLinkMutation.error ?? entitlementMutation.error ?? endMutation.error ?? requestAction.error
              ...
              {pendingInvitations.map((invite) => (
                <div key={invite.id} className="flex flex-col gap-3 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Delivery: {invite.deliveryState ?? 'Not queued'} · {invite.deliveryAttemptCount} attempt{invite.deliveryAttemptCount === 1 ? '' : 's'}{invite.deliveryError ? ` · ${invite.deliveryError}` : ''}</p>
                  </div>
...
function ReadinessChecklist({ readiness }: { readiness: OperationalReadiness }) {
  return <section className="rounded-lg border p-4" aria-labelledby="pseq-readiness-heading"><div className="flex flex-wrap items-center justify-between gap-2"><h2 id="pseq-readiness-heading" className="font-medium">PSeq operational readiness</h2><Badge variant={readiness.status === 'Ready' ? 'secondary' : 'outline'}>{readiness.status}</Badge></div><p className="mt-2 text-sm text-muted-foreground">{readiness.canStageOrder ? 'Internal staging is available.' : 'Internal staging is blocked.'} {readiness.canIssueQuoteOrCommit ? 'Quotes and customer commitment are available.' : 'Quote issuance and customer commitment remain blocked.'}</p>{readiness.blockers.length ? <ul className="mt-3 space-y-2">{readiness.blockers.map((blocker) => <li key={blocker.code} className="rounded-md bg-muted/50 p-3 text-sm"><span className="font-medium">{blocker.label}</span><span className="block text-muted-foreground">{blocker.nextAction}</span></li>)}</ul> : <p className="mt-3 text-sm">All required account, service, order, shipping, and billing configuration is complete.</p>}</section>
}
...
function NotFound() { return <main className="page-wrap px-4 py-8"><Card className="max-w-2xl"><CardHeader><CardTitle>Account not found</CardTitle><CardDescription>The selected account could not be loaded.</CardDescription></CardHeader><CardContent><Button asChild variant="outline"><Link to="/customers"><ArrowLeft data-icon="inline-start" />Back to Portal accounts</Link></Button></CardContent></Card></main> }
