...
const mocks = vi.hoisted(() => ({ list: vi.fn(), get: vi.fn(), create: vi.fn(), update: vi.fn(), deactivate: vi.fn(), navigate: vi.fn(), inventory: vi.fn(), receive: vi.fn(), resumeKitOrder: false, staff: false, admin: true, departmentAdmin: false, wrongDepartment: false, shipmentId: undefined as string | undefined, returnOrderId: undefined as string | undefined }))
...
vi.mock('@tanstack/react-router', () => ({ useNavigate: () => mocks.navigate, useSearch: () => ({ organizationId: '10000000-0000-4000-8000-000000000002', departmentId: '10000000-0000-4000-8000-000000000003', shipmentId: mocks.shipmentId, returnOrderId: mocks.returnOrderId, resumeKitOrder: mocks.resumeKitOrder }), Link: ({ children, to, params, search }: { children: ReactNode; to: string; params?: Record<string, string>; search?: Record<string, unknown> }) => <a data-search={JSON.stringify(search)} href={Object.entries(params ?? {}).reduce((path, [key, value]) => path.replace(`$${key}`, value), to)}>{children}</a> }))
...
describe('customer department delivery locations', () => {
  ...
  it.each([false, true])('resumes an unfinished kit review after address setup, embedded=%s', async embedded => {
    mocks.shipmentId = '30000000-0000-4000-8000-000000000001'
    mocks.returnOrderId = embedded ? '30000000-0000-4000-8000-000000000002' : undefined
    mocks.resumeKitOrder = true
    mount(<DeliveryLocationDetailPage locationId={location.id} />)
    await screen.findByText('100 Science Avenue')
    const link = screen.getByRole('link', { name: embedded ? 'Return to Lab Job' : 'Return to shipment' })
    expect(JSON.parse(link.getAttribute('data-search')!)).toMatchObject({ orderKits: true })
    expect(mocks.receive).not.toHaveBeenCalled()
  })
