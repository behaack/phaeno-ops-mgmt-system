    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>{organization ? 'Capture the business request for review. Approval will not silently provision access, services, or an order.' : 'Submit an exceptional migration or recovery request for review. Submission creates no account, user, service, or order; the later approval explicitly controls account creation and Customer ordering authorization.'}</DialogDescription>
        ...
        <form id="relationship-request" className="grid gap-4" noValidate onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <div className="grid gap-1.5"><Label htmlFor="request-type">Request type <Required /></Label><select id="request-type" className={selectClass} {...form.register('requestType')}><option value="Onboarding">Onboarding</option><option value="Evaluation">Evaluation</option>{organization ? <><option value="ServiceChange">Service change</option><option value="RelationshipChange">Relationship change</option><option value="SalesAssistedOrder">Sales-assisted order</option><option value="Offboarding">Offboarding</option></> : null}</select></div>
