...
export function StandardKitInventoryPanel({ apiEnabled, shipmentId }: { apiEnabled: boolean; shipmentId?: string }) {
  ...
  return <Card id="standard-kits"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Standard kits</CardTitle><CardDescription>Prepare stock at Phaeno, fulfill location deliveries, and track Customer receipt and use.</CardDescription></div><Button disabled={!definitions.data || Boolean(definitions.error)} onClick={() => setPreparing(true)}><Plus aria-hidden="true" />Prepare standard kit</Button></div></CardHeader><CardContent className="space-y-4">
