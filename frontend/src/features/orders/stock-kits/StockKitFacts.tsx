...
export function StockKitFacts({ kit }: { kit: ShippingStockKit }) {
  ...
  return <div className="grid items-start gap-5 lg:grid-cols-2">
    ...
    <div className="space-y-5">
      <Card><CardHeader><CardTitle>Physical container</CardTitle></CardHeader><CardContent className="space-y-4"><dl className="divide-y text-sm"><Fact label="Container barcode"><span className="font-mono">{kit.kitNumber}</span></Fact><Fact label="Tube supplier">{kit.tubeSupplierName}</Fact><Fact label="Tube product">{kit.tubeProductNumber}{kit.tubeProductDescription ? ` — ${kit.tubeProductDescription}` : null}</Fact><Fact label="Tube lot">{kit.tubeLotNumber || 'Not specified'}</Fact><Fact label="Shipper supplier">{kit.shipperSupplierName}</Fact><Fact label="Shipper product">{kit.shipperProductNumber}{kit.shipperProductDescription ? ` — ${kit.shipperProductDescription}` : null}</Fact></dl></CardContent></Card>
      <Card><CardHeader><CardTitle>Product expiration records</CardTitle></CardHeader><CardContent>{kit.productExpirations?.length ? <dl className="divide-y text-sm">{kit.productExpirations.map(product => <Fact key={product.supplierProductId} label={`${product.supplierName} · ${product.productNumber}`}>{product.expirationDate ? `${product.expirationDate}${product.expirationDate < new Date().toISOString().slice(0, 10) ? ' · Expired' : ''}` : product.canExpire ? 'Expiration unknown' : 'Expiration not required when recorded'}</Fact>)}</dl> : <p className="text-sm text-muted-foreground">Expiration requirements and dates were not recorded for this historical kit.</p>}</CardContent></Card>
      <Card><CardHeader><CardTitle>Physical tube roster</CardTitle><p className="text-sm text-muted-foreground">{kit.tubes.length} of {kit.container.capacity} tubes registered</p></CardHeader><CardContent>{kit.tubes.length ? <RegisteredTubeList tubes={kit.tubes} /> : <p className="text-sm text-muted-foreground">Register the permanent supplier barcode on each physical tube.</p>}<p className="mt-3 text-sm">{kit.tubesVerifiedAt ? `Verified ${containerDateTime(kit.tubesVerifiedAt)}` : kit.fulfilledAt ? 'Historical kit — roster verification was not recorded' : 'Physical roster not yet verified'}</p><p className="mt-2 text-sm text-muted-foreground">{atPhaeno ? state === 'Ready' ? 'The verified kit is ready for dispatch.' : 'Register every tube, then rescan the physical roster before dispatch.' : 'The registered tube membership is frozen after dispatch.'}</p>{kit.tubeCorrections?.length ? <div className="mt-4"><h3 className="text-sm font-medium">Corrections</h3><ul className="mt-2 space-y-2 text-xs text-muted-foreground">{kit.tubeCorrections.map((item, index) => <li key={`${item.correctedAt}-${index}`} className="wrap-anywhere">{item.previousBarcode} → {item.replacementBarcode} · {containerDateTime(item.correctedAt)} · {item.reason}</li>)}</ul></div> : null}</CardContent></Card>
    </div>
  </div>
}
