...
export function StandardKitDetailPage({ kitId }: { kitId: string }) {
  ...
  async function saved() { setAction(null); await Promise.all([client.invalidateQueries({ queryKey: ['shipping-stock-kit', kitId] }), client.invalidateQueries({ queryKey: ['shipping-stock-kits'] }), client.invalidateQueries({ queryKey: ['sample-shipping-workflow'] })]) }
