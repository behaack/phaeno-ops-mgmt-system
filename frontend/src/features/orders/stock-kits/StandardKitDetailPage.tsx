...
import { getShippingStockKit, withdrawShippingStockKit, type ShippingStockKit } from '#/api/shipping-containers'
...
export function StandardKitDetailPage({ kitId }: { kitId: string }) {
  ...
  const [action, setAction] = useState<{ kind: 'register' | 'verify' | 'correct' | 'dispatch' | 'legacy-dispatch' | 'barcode' | 'withdraw'; kit: ShippingStockKit } | null>(null)
  const [withdrawalReason, setWithdrawalReason] = useState('')
  const withdraw = useMutation({ mutationFn: (kit: ShippingStockKit) => withdrawShippingStockKit(kit.id, kit.version, withdrawalReason.trim()), onSuccess: async () => { setAction(null); setWithdrawalReason(''); await refreshStockKitSupply(client) } })
  ...
  if (!kit.withdrawnAt) availableActions.push({ kind: 'withdraw', label: 'Withdraw physical kit' })
  return <main className="page-wrap px-4 py-8">{back}<header className="my-5 flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><h1 className="wrap-anywhere text-2xl font-semibold">{kit.kitNumber}</h1><p className="mt-2 wrap-anywhere text-sm text-muted-foreground">{kit.container.commonName} · SKU {kit.container.sku} · {kit.container.capacity} tube capacity</p><Badge variant="outline" className="mt-2">{stockKitStatus(stockKitState(kit))}</Badge></div>{availableActions.length === 1 ? <Button type="button" variant="outline" disabled={Boolean(query.error)} onClick={() => setAction({ kind: availableActions[0].kind, kit })}>{availableActions[0].label}</Button> : <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline">Actions</Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-max max-w-[calc(100vw-2rem)]">{availableActions.map(item => <DropdownMenuItem key={item.kind} disabled={Boolean(query.error) && item.kind !== 'barcode'} onSelect={() => setAction({ kind: item.kind, kit })}>{item.label}</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>}</header>
    ...
    <Dialog open={action?.kind === 'withdraw'} onOpenChange={open => { if (!open && !withdraw.isPending) setAction(null) }}><DialogContent showCloseButton={!withdraw.isPending}><DialogHeader><DialogTitle>Withdraw physical kit?</DialogTitle><DialogDescription>{action?.kind === 'withdraw' ? action.kit.kitNumber : 'This kit'} will be blocked from dispatch, reservation, and new shipment use. Existing issued instructions keep their saved facts.</DialogDescription></DialogHeader><div><Label htmlFor="stock-kit-withdrawal-reason"><RequiredFieldName>Reason</RequiredFieldName></Label><Textarea id="stock-kit-withdrawal-reason" className="mt-2" rows={3} maxLength={1000} value={withdrawalReason} onChange={event => setWithdrawalReason(event.target.value)} disabled={withdraw.isPending} /></div>{withdraw.error ? <Alert variant="destructive"><AlertTitle>Kit was not withdrawn</AlertTitle><AlertDescription>{getOrderErrorMessage(withdraw.error, 'Refresh the physical kit and try again.')}</AlertDescription></Alert> : null}<RequiredDialogFooter><Button variant="outline" disabled={withdraw.isPending} onClick={() => setAction(null)}>Cancel</Button><Button variant="destructive" disabled={withdraw.isPending || !withdrawalReason.trim()} onClick={() => { if (action?.kind === 'withdraw') withdraw.mutate(action.kit) }}>{withdraw.isPending ? 'Withdrawing…' : 'Withdraw kit'}</Button></RequiredDialogFooter></DialogContent></Dialog>
