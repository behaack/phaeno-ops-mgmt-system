...
export function matchingUnlinkedKitRequest(kit: ShippingStockKit, requests: TransportationKitRequest[]) {
  if (kit.status !== 'Fulfilled' || kit.boundSampleShipmentId || !kit.authorizationSourceId || !kit.organizationId
  ...
  const matches = requests.filter(request => request.jobId === kit.authorizationSourceId
