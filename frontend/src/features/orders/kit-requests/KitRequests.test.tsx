...
describe('transportation-kit fulfillment', () => {
  ...
  it('shows a frozen address and stock shortage while explaining a blocked dispatch', async () => { mocks.get.mockResolvedValue({ ...detail, availableStockKits: [], canDispatch: false, dispatchBlockedReason: 'Prepare fully registered kits first.' }); mount(<KitRequestDetailPage requestId={request.id} />); expect(await screen.findByText('100 Science Avenue')).toBeTruthy(); expect(screen.getByText('Prepare kits before shipping')).toBeTruthy(); expect(screen.getByText('Prepare fully registered kits first.')).toBeTruthy(); expect(screen.queryByRole('button', { name: 'Record kit shipment' })).toBeNull(); expect(screen.getByRole('button', { name: 'Prepare kits' })).toBeTruthy() })
  it('keeps partial shipment available and shows only the actual missing quantity', async () => {
    mocks.get.mockResolvedValue({ ...detail, availableStockKits: [detail.availableStockKits[0]] })
    mount(<KitRequestDetailPage requestId={request.id} />)
    expect(await screen.findByRole('button', { name: 'Record kit shipment' })).toHaveProperty('disabled', false)
    expect(screen.getByRole('button', { name: 'Prepare kits' })).toBeTruthy()
  ...
  it('does not offer shipment for stock of an unrelated size', async () => {
    mocks.get.mockResolvedValue({ ...detail, availableStockKits: [{ ...detail.availableStockKits[0], containerDefinitionId: 'other-size' }] })
    mount(<KitRequestDetailPage requestId={request.id} />)
  ...
  it('retains an open fulfillment draft when its request refresh fails', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Record kit shipment' })); fill('Tracking number', 'RETAIN-THIS')
