...
import { getOrderConfiguration, getOrderErrorMessage, getPlatformLabIntake, getPlatformOrder, listIntegrationMessages, listNotificationMessages, listPlatformOrders, retryIntegrationMessage, retryNotificationMessage, runPlatformAction, updateOperationalAssignment, type DataAssemblyRequest, type IntegrationMessage, type LabServiceOrder, type NotificationMessage, type PagedResult, type ReagentOrder } from '#/api/order-management'
...
function OperationalDetail({ workflow, orderId, apiEnabled, userId }: { workflow: Workflow; orderId: string; apiEnabled: boolean; userId: string | null }) {
  ...
  const labIntake = useMutation({
    mutationFn: () => getPlatformLabIntake(orderId),
    onSuccess: async (intake) => {
      await navigate({ to: '/lab-operations/$workOrderId', params: { workOrderId: intake.workOrderId }, search: { section: undefined } })
    },
  })
