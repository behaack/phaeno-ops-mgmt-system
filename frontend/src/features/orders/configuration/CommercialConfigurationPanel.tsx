...
export function CommercialConfigurationPanel({ configuration }: { configuration: OrderConfiguration }) {
  ...
  return <><Card><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>Legacy credit and accounting links</CardTitle><CardDescription>These settings remain for partner kit, data-assembly, and historical QuickBooks workflows. PSeq billing and tax are managed in Finance, and PSeq result release is never payment-gated.</CardDescription></div><Button type="button" onClick={() => open(null)}><Plus data-icon="inline-start" />Add legacy link</Button></div></CardHeader><CardContent><div className="divide-y">{configuration.commercialProfiles.map((profile) => <button type="button" key={profile.id} onClick={() => open(profile)} className="flex w-full cursor-pointer items-center justify-between gap-3 py-3 text-left focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none"><div><span className="font-medium">{profile.organizationName}</span><span className="mt-1 block text-xs text-muted-foreground">QuickBooks customer {profile.qboCustomerId ?? 'not linked'}</span></div><div className="flex flex-wrap gap-2"><Badge variant={profile.labCreditApproved ? 'secondary' : 'outline'}>Legacy lab {profile.labCreditApproved ? 'credit' : 'prepay'}</Badge><Badge variant={profile.assemblyCreditApproved ? 'secondary' : 'outline'}>Assembly {profile.assemblyCreditApproved ? 'credit' : 'prepay'}</Badge></div></button>)}</div>{!configuration.commercialProfiles.length ? <p className="py-8 text-center text-sm text-muted-foreground">No legacy commercial links configured.</p> : null}</CardContent></Card>
