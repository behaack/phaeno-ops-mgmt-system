...
export function ContainerSizesPanel({ apiEnabled, configuration }: { apiEnabled: boolean; configuration: SampleShippingConfiguration }) {
  ...
  return <Card id="container-sizes" className="gap-0 py-0"><CardHeader className="border-b bg-muted/50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Container sizes</CardTitle><CardDescription>Approved sizes, usable tube capacities, and compatible handling rules. Open a size to review its revisions.</CardDescription></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!apiEnabled || !query.data} onClick={() => setPreview(null)}>Preview recommendation</Button><Button disabled={!apiEnabled} onClick={() => setEditing(null)}><Plus aria-hidden="true" />Add container size</Button></div></div>
