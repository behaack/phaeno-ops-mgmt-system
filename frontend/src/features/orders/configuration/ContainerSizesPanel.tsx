...
export function ContainerSizesPanel({ apiEnabled, configuration }: { apiEnabled: boolean; configuration: SampleShippingConfiguration }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { void navigate({ to: '/sample-shipping-settings', search: { ...search, shippingSection: 'containers', containerSearch: searchText || undefined, containerPage: 1 }, replace: true, resetScroll: false }) }, 200)
    return () => window.clearTimeout(timer)
  }, [navigate, search, searchText])
  function changeSearch(update: ShippingContainerListSearch) {
    void navigate({ to: '/sample-shipping-settings', search: { ...search, ...update, shippingSection: 'containers' }, replace: true, resetScroll: false })
  }
  ...
  return <Card id="container-sizes" className="gap-0 py-0"><CardHeader className="border-b bg-muted/50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Kit specifications</CardTitle><CardDescription>Approved kit products, usable tube capacities, and sample and destination assignments. Open a specification to review its revisions.</CardDescription></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!apiEnabled || !query.data || !configuration.instructionRules.length} onClick={() => setPreview(null)}>Preview recommendation</Button><Button disabled={!apiEnabled || !configuration.instructionRules.length} onClick={() => setEditing(null)}><Plus aria-hidden="true" />Add kit specification</Button></div></div>
    <div className="col-span-full mt-3 flex flex-wrap items-end gap-3"><div className="min-w-48 flex-1"><Label htmlFor="container-search">Search kit specifications</Label><Input id="container-search" className="mt-2" value={searchText} onChange={event => setSearchText(event.target.value)} placeholder="Common name or SKU" /></div><div><Label htmlFor="container-status">Status</Label><select id="container-status" className="mt-2 h-9 w-full cursor-pointer rounded-md border border-input bg-background px-3 text-sm focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none" value={search.containerStatus} onChange={event => changeSearch({ containerStatus: event.target.value as ShippingContainerListSearch['containerStatus'], containerPage: 1 })}><option value="all">All statuses</option><option value="active">Active now</option><option value="inactive">Inactive, scheduled, or ended</option></select></div>{needle || search.containerStatus !== 'all' ? <Button variant="ghost" onClick={() => { setSearchText(''); changeSearch({ containerSearch: undefined, containerStatus: 'all', containerPage: 1 }) }}>Clear all</Button> : null}</div>
  </CardHeader><CardContent className="space-y-4 p-4">
    {!configuration.instructionRules.length ? <Alert><AlertTitle>Add a shipping assignment first</AlertTitle><AlertDescription>Each container needs an approved sample and destination combination. <Link className="underline" to="/sample-shipping-settings" search={{ shippingSection: 'instructions' }}>Go to Shipping assignments</Link> to complete the setup before adding a size or previewing a recommendation.</AlertDescription></Alert> : null}
    ...
    {query.data && !items.length ? <p className="py-6 text-center text-sm text-muted-foreground">{latest.length ? 'No kit specifications match these filters.' : 'No kit specifications are configured. Add a named Phaeno kit product and its approved shipping specification.'}</p> : null}
