...
export function ShippingContainerDetailPage({ containerId }: { containerId: string }) {
  ...
  return <main className="page-wrap px-4 py-8"><Link className="inline-flex items-center gap-1 text-sm text-primary underline underline-offset-2" to="/sample-shipping-settings" search={{ ...parseShippingContainerListSearch(search), shippingSection: 'containers' }} hash="container-sizes"><ArrowLeft aria-hidden="true" className="size-4" />Back to container sizes</Link>
    <header className="my-5 flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="wrap-anywhere text-2xl font-semibold">{item.commonName}</h1><Badge variant="outline">{containerEffectiveState(item)}</Badge></div><p className="mt-2 wrap-anywhere text-sm text-muted-foreground">SKU {item.sku} · revision {item.revision} · {item.tubeCapacity} tubes</p></div><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline">Actions</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={!configuration.data || !catalog.data} onSelect={() => setPreview(item)}>Preview recommendation</DropdownMenuItem><DropdownMenuItem disabled={!configuration.data || !isLatest} onSelect={() => setEditing(item)}>Create revision</DropdownMenuItem>{item.isActive && containerEffectiveState(item) !== 'Ended' ? <DropdownMenuItem variant="destructive" onSelect={() => { deactivate.reset(); setDeactivation(item) }}>Deactivate revision</DropdownMenuItem> : null}</DropdownMenuContent></DropdownMenu></header>
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(18rem,1fr)]"><div className="space-y-5"><Card><CardHeader><CardTitle>Container details</CardTitle></CardHeader><CardContent><dl className="divide-y text-sm"><Fact label="Usable tube capacity" value={`${item.tubeCapacity} tubes`} /><Fact label="Supplier" value={item.supplierName ?? 'Not specified'} /><Fact label="Supplier product number" value={item.supplierProductNumber ?? 'Not specified'} /><Fact label="Display order" value={String(item.displayOrder)} /><Fact label="Effective from" value={containerDateTime(item.effectiveFrom)} /><Fact label="Effective through" value={item.effectiveTo ? containerDateTime(item.effectiveTo) : 'No end date'} /></dl></CardContent></Card>
      <Card><CardHeader><CardTitle>Sample and destination assignments</CardTitle></CardHeader><CardContent><ul className="divide-y text-sm">{item.compatibilities.map(pair => { const rule = configuration.data?.instructionRules.find(value => value.id === pair.instructionRuleId); return <li key={`${pair.sampleTypeDefinitionId}-${pair.instructionRuleId}`} className="py-3">{rule ? <><p className="font-medium">{rule.sampleTypeName} · {rule.destinationName}</p><p className="mt-1 text-xs text-muted-foreground">{rule.compatibilityGroup} · rule revision {rule.revision}{rule.isActive ? '' : ' · inactive'}</p><dl className="mt-3 space-y-3"><div><dt className="font-medium">Temperature control for this container</dt><dd className="mt-1 whitespace-pre-wrap wrap-anywhere">{pair.temperatureControlInstructions || 'Not recorded for this combination.'}</dd></div><div><dt className="font-medium">Packing steps for this combination</dt><dd className="mt-1 whitespace-pre-wrap wrap-anywhere">{pair.packingInstructions || 'Not recorded for this combination.'}</dd></div></dl></> : <p className="text-muted-foreground">{configuration.isLoading ? 'Loading shipping assignment…' : 'Referenced shipping assignment is unavailable.'}</p>}</li> })}</ul>{!item.compatibilities.length ? <p className="text-sm text-muted-foreground">This container cannot be selected for shipping until at least one sample and destination assignment is configured. Active status alone does not make it available.</p> : null}</CardContent></Card>
