...
export function TrayFormatList() {
  ...
  if (query.isPending) return <p role="status">Loading library tray formats…</p>
  if (query.isError) return <div className="space-y-3"><p role="alert">{getLabOperationsError(query.error, 'Library tray formats could not be loaded.')}</p><Button variant="outline" onClick={() => void query.refetch()}>Reload library tray formats</Button></div>

  const { formats, canConfigure } = query.data
  return <>
    <PreparationPanel
      title="Library tray formats"
      description="Define library tray layouts for library preparation, including position labels and unavailable positions. Existing batches retain their original layout."
      actions={canConfigure ? <Button type="button" onClick={() => setDialog('create')}><Plus data-icon="inline-start" /> New library tray format</Button> : undefined}
    >
      {formats.length ? <ul aria-label="Library tray formats" className="space-y-3">
        {formats.map(format => <li key={format.id} className={`${prepRowClass} flex flex-wrap items-center justify-between gap-3`}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {canConfigure ? <PreparationActions items={[{ label: 'Edit', onClick: () => setDialog(format) }]} /> : null}
          </div>
        </li>)}
      </ul> : <p className="py-4 text-sm text-muted-foreground">{canConfigure ? 'No library tray formats yet. Create a format to make it available when assembling a preparation batch.' : 'No library tray formats yet. A Supervisor or Protocol Administrator can configure them.'}</p>}
    </PreparationPanel>
