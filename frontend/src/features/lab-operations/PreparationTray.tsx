...
export function PreparationTray({ batch, pending, onScan, onTrayScan, onConfirmTray, children, eligibleTubes }: {
  ...
  return <Card className="gap-0 overflow-hidden py-0">
    <CardHeader className={`${expanded ? 'border-b' : ''} bg-muted/50 p-4`}>
      <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3">{batch.trayBarcode ? <QRCodeSVG value={batch.trayBarcode} level="M" marginSize={4} size={64} bgColor="#ffffff" fgColor="#000000" role="img" aria-label={`Tray barcode ${batch.trayBarcode}`} className="size-16 shrink-0" /> : null}<div className="min-w-0"><CardTitle>{hasStarted ? <button type="button" data-tray-toggle aria-expanded={expanded} aria-controls={contentId} onClick={() => setExpansion({ started: hasStarted, open: !expanded })} className="flex min-h-9 cursor-pointer items-center gap-2 rounded-sm text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ChevronRight aria-hidden="true" className={`size-4 ${expanded ? 'rotate-90' : ''}`} />Tray{' '}<span className="text-sm font-normal text-muted-foreground">({batch.members.length} {batch.members.length === 1 ? 'tube' : 'tubes'})</span></button> : <>Tray{batch.trayConfirmed && batch.status === 'Draft' ? ' · Confirmed' : ''}</>}</CardTitle>{!editable && batch.trayBarcode ? <p className="wrap-anywhere text-sm text-muted-foreground">Physical tray: {batch.trayBarcode}</p> : null}</div></div><div className="flex flex-wrap gap-2">{batch.trayBarcode ? <Button size="sm" variant="outline" disabled={busy} onClick={() => setPrintOpen(true)}><Printer aria-hidden="true" />Print tray label</Button> : null}{editable && trayReady && batch.members.length > 0 && onConfirmTray ? <Button size="sm" disabled={busy || dirty} onClick={onConfirmTray}>Confirm tray</Button> : null}</div></div>
      <CardDescription hidden={!expanded}>{editable ? trayReady ? 'Scan tubes into empty positions. Review the contents and choose Confirm tray when assembled. Partial trays are permitted.' : 'Scan and save the physical tray barcode once, then load the tubes. The saved tray is remembered when you return.' : batch.status === 'Draft' && batch.trayConfirmed ? 'The assembled tray is confirmed and locked. Start preparation when ready, or choose Edit tray to revise it.' : batch.status === 'Draft' ? 'Draft tray positions are shown for review.' : 'Positions remain fixed for the duration of this batch.'}</CardDescription>
      ...
      <div className="overflow-x-auto" role="region" tabIndex={0} aria-label="Tray positions"><div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${batch.layout.columns}, minmax(12rem, 1fr))` }}>
        {positions.map(position => {
          ...
          if (tube) return <button key={position} type="button" aria-pressed={selectedMemberId === tube.id} aria-controls="preparation-tube-details"
            onClick={() => setSelectedMemberId(tube.id)} aria-label={`View ${position}, tube ${tube.barcode}${tube.state === 'Failed' ? ', Failed' : ''}`}
            ...
            <strong className="absolute left-3 top-3">{position}</strong>
              <QRCodeSVG value={tube.barcode} level="M" marginSize={4} size={64} bgColor="#ffffff" fgColor="#000000" aria-hidden="true" className="size-16 shrink-0" />
