...
export function PreparationBatchPage({ batchId }: { batchId: string }) {
  ...
  if (progress.phase === 0 && data.trayBarcode && data.members.length > 0) {
    ...
    nextDescription = 'Record the current step and resolve its observations and QC before moving forward.'
    ...
    if (current && progress.nextStep) {
      ...
      nextDescription = `${current.name}. Record the required evidence for the covered tubes.${step.requiredRole && !data.roles.includes(step.requiredRole) ? ` Requires ${step.requiredRole}.` : ''}`
      ...
      nextDescription = progress.finalStage ? 'All required evidence and output identities are recorded. Complete this protocol to establish the library outcomes.' : 'Required evidence is resolved. Complete this protocol to continue to the next one.'
