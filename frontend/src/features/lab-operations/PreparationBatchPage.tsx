...
import { listMasterMixes, masterMixesKey } from '#/api/lab-master-mix'
...
export function PreparationBatchPage({ batchId }: { batchId: string }) {
  ...
  const masterMixes = useQuery({ queryKey: masterMixesKey, queryFn: listMasterMixes, enabled: canAccess && Boolean(query.data) })
  ...
  const [selectedTube, setSelectedTube] = useState<{ id: string; barcode: string; jobName: string } | null>(null)
  ...
  const save = useMutation({ mutationFn: async ({ report, ...input }: PreparationSaveInput) => {
    ...
    if (submitted.action === 'add') setSelectedTube(null)
    ...
    await Promise.allSettled([client.invalidateQueries({ queryKey: ['lab-preparation-tubes', batchId] }), client.invalidateQueries({ queryKey: ['lab-operations'] }), client.invalidateQueries({ queryKey: masterMixesKey }), client.invalidateQueries({ queryKey: ['lab-attempts'] })])
    ...
    await Promise.allSettled([client.invalidateQueries({ queryKey: ['lab-preparation', batchId] }), client.invalidateQueries({ queryKey: masterMixesKey })])
  ...
  const usesMasterMix = data.stages.some(stage => stage.definition.steps.some(step => step.captures.some(capture => capture.type === 'material' && capture.material?.masterMixWorkflowId)))
  const eligibleTubes = editable ? <details className="group/tubes border-t pt-2">
      ...
      <div className="space-y-3 pt-2"><p className="text-sm text-muted-foreground">Accepted tubes from compatible jobs, including compatible unstarted source selections. Select the intended tube, then scan its physical barcode into the tray.</p>
      {selectedTube ? <p role="status" className="text-sm">Selected: {selectedTube.barcode} · {selectedTube.jobName}. Scan this tube into an empty tray position. <Button type="button" size="sm" variant="outline" onClick={() => setSelectedTube(null)}>Clear selection</Button></p> : null}
      ...
      {tubes.isError ? <p role="alert">{getLabOperationsError(tubes.error, 'Tubes could not be loaded.')}</p> : tubes.isPending ? <p role="status" className="text-sm text-muted-foreground">Loading eligible tubes…</p> : tubes.data?.items.length ? tubes.data.items.map(t => <div key={t.id} className={`${prepRowClass} text-sm`}><strong>{t.barcode}</strong><p>{t.jobName} · {t.specimenName}</p><p className="text-muted-foreground">Freezer box: {t.location || 'Not recorded'}</p><Button type="button" size="sm" variant="outline" className="mt-2" aria-pressed={selectedTube?.id === t.id} onClick={() => setSelectedTube({ id: t.id, barcode: t.barcode, jobName: t.jobName })}>{selectedTube?.id === t.id ? 'Selected tube' : 'Select tube for scan'}</Button></div>) : <p role="status" className="text-sm text-muted-foreground">{search.trim() || freezerBox.trim() ? 'No eligible tubes match these filters. Change the search or clear the filters.' : 'No eligible tubes found for this service. Check accession, tube acceptance and source reservations. An existing attempt must use this batch’s workflow version.'}</p>}
  ...
  return <main className="page-wrap space-y-5 px-4 py-6 sm:px-6">
    ...
    <PreparationTray key={batchId} onConfirmTray={() => open({ key: 'confirm-tray' })} eligibleTubes={eligibleTubes} batch={data} pending={save.isPending || handoff.isPending} onScan={(position, barcode) => save.mutateAsync({ action: 'add', position, barcode, containerId: selectedTube?.barcode === barcode ? selectedTube.id : undefined })} onTrayScan={barcode => save.mutateAsync({ action: 'assign-tray', barcode })}>
      {selectedMemberId => data.members.filter(m => m.id === selectedMemberId).map(m => <section key={m.id} aria-label={`Tube details for ${m.position}`} className={prepRowClass}><h3 className="text-sm"><strong>{m.position} · {m.barcode}</strong> · {m.jobName} · {human(m.state)}{m.state !== 'Failed' && m.output && !m.output.confirmed ? ' · Output scan needed' : ''}</h3><div className="mt-4 space-y-3 text-sm">
        ...
        {m.libraryTube ? <div><p>Library tube in {m.position}: <Link className="break-all underline" to="/lab-operations/$workOrderId/containers/$containerId" params={{ workOrderId: m.workOrderId, containerId: m.libraryTube.id }} search={{ section: 'work' }}>{m.libraryTube.barcode}</Link></p><p className="text-muted-foreground">{m.libraryTube.transferId ? 'Physical transfer recorded. Prepared yield is measured separately.' : m.libraryTube.barcodeSource === 'PhaenoGenerated' ? 'Open the library tube to print its label and scan the printed label back before recording the Biological material step.' : 'Manufacturer barcode assigned. Scan the physical tube when recording the Biological material step.'}</p></div> : tracksBiologicalMaterial && !m.output ? <p className="text-muted-foreground">A separate library tube must be assigned before recording the Biological material step.</p> : null}
    ...
    {usesMasterMix ? <PreparationPanel title="Master mix" description="A single prepared mix can serve this tray and other library trays. Record each tray's actual amount in its approved step."><div className="space-y-2 text-sm">{data.masterMixUses?.length ? <ul className="space-y-2">{data.masterMixUses.map(use => <li key={`${use.labPreparationRecordId}-${use.fieldKey}`} className={prepRowClass}><Link className="font-medium text-primary underline" to="/lab-operations/master-mixes/$mixId" params={{ mixId: use.preparationId }}>{use.workflowName} · revision {use.workflowRevision}</Link><p>{use.quantityText} {use.quantityUnit} {use.voidedAtUtc ? 'record voided after verification of no physical use' : use.correctionAction === 'Discrepancy' ? 'physically used or uncertain; discrepancy retained' : 'used'} · {new Date(use.recordedAtUtc).toLocaleString()}</p>{use.correctionReason ? <p className="text-muted-foreground">Correction: {use.correctionReason}</p> : null}</li>)}</ul> : <p>No master mix has been used on this tray yet.</p>}<Link className="inline-block text-primary underline" to="/lab-operations" search={{ section: 'master-mixes' }}>Open Master mixes</Link></div></PreparationPanel> : null}
    ...
    {action?.key === 'allocate-library-tube' && member ? <PreparationLibraryTubeDialog member={member} suppliers={resources.data?.suppliers ?? []} pending={save.isPending} error={error} onClose={() => setAction(null)} onSubmit={value => save.mutate({ action: 'allocate-library-tube', memberId: member.id, ...value })} /> : null}
