...
const api = vi.hoisted(() => ({ packet: vi.fn(), tube: vi.fn(), identity: vi.fn(), queue: vi.fn(), receive: vi.fn(), work: vi.fn(), accession: vi.fn(), batch: vi.fn() }))
vi.mock('#/api/lab-operations', () => ({ getLabWorkOrder: api.work, accessionShipmentTube: api.accession, acceptRemainingLabTubes: api.batch, getLabOperationsError: (_: unknown, fallback: string) => fallback, getLabIntakeReasons: async () => [{ code: 'damaged_container', label: 'Damaged container' }] }))
...
describe('LabReceiptAccessionPanel navigation', () => {
  ...
  it('records a broken expected tube without storage, then accepts only identified undecided tubes', async () => {
    ...
    await screen.findByRole('dialog', { name: 'Accession tubes in SHIP-1' })
    const exception = screen.getByRole('button', { name: 'Record exception for TUBE-1' })
    ...
    const tube = screen.getByLabelText('Supplier tube barcode')
    fireEvent.change(tube, { target: { value: 'TUBE-2' } }); fireEvent.submit(tube.closest('form')!)
    expect(await screen.findByText('1 with exceptions | 1 to be accepted')).toBeTruthy()
    fireEvent.click(await screen.findByRole('button', { name: 'Accept (1)' }))
    const box = await screen.findByLabelText(/Freezer box barcode/)
    fireEvent.submit(box.closest('form')!)
    expect(await screen.findByText('Scan the actual freezer box for this tube.')).toBeTruthy()
    expect(api.batch).not.toHaveBeenCalled()
    fireEvent.change(box, { target: { value: 'BOX-2' } })
    fireEvent.click(screen.getByRole('checkbox', { name: /I inspected every listed tube/ }))
    fireEvent.submit(box.closest('form')!)
    ...
    expect(screen.getByText('1 with exceptions | 0 to be accepted')).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Accept (0)' })).toHaveProperty('disabled', true)
    expect(api.receive).not.toHaveBeenCalled()
  })
  ...
  it('keeps an unexpected tube out of the freezer-box prompt', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: 'Identify tube' })).not.toHaveProperty('disabled', true))
  ...
  it('guards unsaved bulk storage entries before cancelling acceptance', async () => {
    ...
    try {
      ...
      fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
      expect(screen.queryByLabelText(/Freezer box barcode/)).toBeNull()
      expect(screen.getByRole('button', { name: 'Accept (1)' })).toBeTruthy()
