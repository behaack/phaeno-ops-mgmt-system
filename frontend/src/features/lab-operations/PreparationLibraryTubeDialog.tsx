...
const schema = z.object({ barcodeSource: z.enum(['PhaenoGenerated', 'Manufacturer']), barcode: z.string().trim().max(100, 'Use 100 characters or fewer.'), manufacturerSupplierId: z.string() }).superRefine((value, context) => {
  if (value.barcodeSource === 'Manufacturer' && !value.manufacturerSupplierId) context.addIssue({ code: 'custom', path: ['manufacturerSupplierId'], message: 'Select the manufacturer of this physical tube.' })
...
export function PreparationLibraryTubeDialog({ member, suppliers, pending, error, onClose, onSubmit }: {
  member: PreparationMember; suppliers: LabSupplier[]; pending: boolean; error?: string; onClose: () => void;
  onSubmit: (value: { barcodeSource: 'PhaenoGenerated' | 'Manufacturer'; barcode?: string; manufacturerSupplierId?: string }) => void
}) {
  const form = useForm<Values>({ resolver: zodResolver(schema), defaultValues: { barcodeSource: 'PhaenoGenerated', barcode: '', manufacturerSupplierId: '' } })
  ...
  return <Dialog open onOpenChange={open => { if (!open) close() }}><DialogContent><form className="contents" onSubmit={form.handleSubmit(value => onSubmit({ barcodeSource: value.barcodeSource, ...(value.barcodeSource === 'Manufacturer' ? { barcode: value.barcode, manufacturerSupplierId: value.manufacturerSupplierId } : {}) }))} noValidate>
