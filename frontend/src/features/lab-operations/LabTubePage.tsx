...
import { z } from 'zod'
import { getLabAttempts, getLabContainerMoves, getLabOperationsError, getLabWorkOrder, moveLabContainer, reviewLabTubeIntake, type LabContainer } from '#/api/lab-operations'
...
export function LabTubePage({ workOrderId, containerId }: { workOrderId: string; containerId: string }) {
  ...
  const moves = useQuery({ queryKey: ['lab-container-moves', containerId], queryFn: () => getLabContainerMoves(containerId), enabled })
  const [action, setAction] = useState<'correct' | 'label' | 'move' | null>(null)
  const tube = work.data?.containers.find(t => t.id === containerId)
  const refresh = async () => { await Promise.all(['lab-work-order', 'lab-attempts', 'lab-execution', 'accession-packet', 'lab-operations', 'lab-container-moves'].map(key => client.invalidateQueries({ queryKey: [key] }))) }
  ...
  const canCorrect = Boolean(session?.capabilities.canSuperviseLabWork && attempts.data && tube.kind === 'SubmittedSpecimen' && !used && ['Available', 'Rejected', 'LabelPending'].includes(tube.status) && !['Cancelled', 'ReadyForRelease'].includes(work.data.workOrder.status) && specimen?.intakeDisposition !== 'Cancelled')
  const canPrint = session?.capabilities.canOperateLabWork && tube.barcodeSource === 'PhaenoGenerated' && tube.status !== 'Rejected'
  const canMove = Boolean(session?.capabilities.canOperateLabWork && (tube.status !== 'Rejected' || tube.location))
  ...
  return <main className="page-wrap space-y-5 px-4 py-8">
    ...
    <header className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="break-all font-mono text-2xl font-semibold">{tube.barcode}</h1><p className="mt-1 text-sm text-muted-foreground">{tube.label} · {human(tube.kind)}</p><div className="mt-2 flex flex-wrap gap-2"><Badge variant="outline">{human(tube.status)}</Badge>{tube.kind === 'SubmittedSpecimen' ? <Badge variant="secondary">Intake: {human(tube.intakeDisposition ?? 'Not recorded')}</Badge> : null}</div></div><PreparationActions items={[...(canCorrect ? [{ label: 'Correct intake decision', onClick: () => setAction('correct') }] : []), ...(canMove ? [{ label: 'Move to scanned location', onClick: () => setAction('move') }] : []), ...(canPrint ? [{ label: tube.labelPrintCount ? 'Reprint label' : 'Print label', onClick: () => setAction('label') }] : [])]} /></header>
    ...
    <Card className="gap-0 py-0"><CardHeader className="border-b bg-muted/50 p-4"><CardTitle>Location history</CardTitle></CardHeader><CardContent className="p-4">{moves.isPending ? <p role="status" className="text-sm">Loading moves…</p> : moves.isError ? <div className="space-y-2"><p role="alert" className="text-sm">{getLabOperationsError(moves.error, 'Location history could not be loaded.')}</p><Button type="button" variant="outline" size="sm" onClick={() => void moves.refetch()}>Retry</Button></div> : moves.data?.length ? <ol className="space-y-2">{moves.data.map(move => <li key={move.id} className="rounded-md border p-3 text-sm"><p className="break-all">{move.previousLocation ?? 'No previous location'} → {move.destinationBarcode}</p><p className="text-xs text-muted-foreground">Recorded {new Date(move.occurredAtUtc).toLocaleString()}</p></li>)}</ol> : <p className="text-sm text-muted-foreground">No moves recorded after accession.</p>}</CardContent></Card>
    ...
    {action === 'move' ? <MoveContainerDialog tube={tube} onClose={() => setAction(null)} onSaved={async () => { setAction(null); await refresh() }} /> : null}
  </main>
}

const moveSchema = z.object({ scannedContainerBarcode: z.string().trim().min(1, 'Scan the selected container.'), scannedDestinationBarcode: z.string().trim().min(1, 'Scan the destination barcode.').max(255, 'Use 255 characters or fewer.'), confirmed: z.boolean() })
  .superRefine((value, context) => {
    if (!value.confirmed) context.addIssue({ code: 'custom', path: ['confirmed'], message: 'Confirm the scanned object and destination.' })
    if (value.scannedDestinationBarcode === value.scannedContainerBarcode) context.addIssue({ code: 'custom', path: ['scannedDestinationBarcode'], message: 'Scan the destination location, not the tube again.' })
  })

function MoveContainerDialog({ tube, onClose, onSaved }: { tube: LabContainer; onClose: () => void; onSaved: () => Promise<void> }) {
  const form = useForm<z.infer<typeof moveSchema>>({ resolver: zodResolver(moveSchema), defaultValues: { scannedContainerBarcode: '', scannedDestinationBarcode: '', confirmed: false } })
  const save = useMutation({ mutationFn: (value: z.infer<typeof moveSchema>) => moveLabContainer(tube.id, { version: tube.version, ...value }), onSuccess: onSaved })
  const dirty = form.formState.isDirty
  const close = () => { if (!save.isPending && (!dirty || window.confirm('Discard the unsaved move?'))) onClose() }
  useBlocker({ shouldBlockFn: () => save.isPending || dirty && !window.confirm('Discard the unsaved move?'), enableBeforeUnload: () => save.isPending || dirty })
  return <Dialog open onOpenChange={open => { if (!open) close() }}><DialogContent><DialogHeader><DialogTitle>Move container</DialogTitle><DialogDescription>Scan the physical container, then the destination freezer box or storage location barcode. Confirm both scans to record its new location.</DialogDescription></DialogHeader>
    <form id="move-container" className="space-y-4" noValidate onSubmit={form.handleSubmit(value => save.mutate(value))}>
      <div><Label htmlFor="move-container-scan"><RequiredFieldName>Scan container</RequiredFieldName></Label><Input id="move-container-scan" className="mt-2" autoComplete="off" spellCheck={false} aria-invalid={Boolean(form.formState.errors.scannedContainerBarcode)} {...form.register('scannedContainerBarcode')} />{form.formState.errors.scannedContainerBarcode ? <p role="alert" className="mt-1 text-sm text-destructive">{form.formState.errors.scannedContainerBarcode.message}</p> : null}</div>
      <div><Label htmlFor="move-destination-scan"><RequiredFieldName>Scan destination barcode</RequiredFieldName></Label><Input id="move-destination-scan" className="mt-2" autoComplete="off" spellCheck={false} maxLength={255} aria-invalid={Boolean(form.formState.errors.scannedDestinationBarcode)} {...form.register('scannedDestinationBarcode')} />{form.formState.errors.scannedDestinationBarcode ? <p role="alert" className="mt-1 text-sm text-destructive">{form.formState.errors.scannedDestinationBarcode.message}</p> : null}</div>
      <p className="text-sm">Current location: {tube.location ?? 'Not recorded'}</p>
      <label className="flex cursor-pointer items-start gap-2 text-sm"><input type="checkbox" className="mt-0.5 size-4 cursor-pointer" aria-invalid={Boolean(form.formState.errors.confirmed)} {...form.register('confirmed')} /><RequiredFieldName>I confirmed the physical container and destination.</RequiredFieldName></label>{form.formState.errors.confirmed ? <p role="alert" className="text-sm text-destructive">{form.formState.errors.confirmed.message}</p> : null}
    </form>{save.error ? <p role="alert" className="text-sm text-destructive">{getLabOperationsError(save.error, 'The move could not be saved. Review both scans and retry.')}</p> : null}<RequiredDialogFooter><Button type="button" variant="outline" disabled={save.isPending} onClick={close}>Cancel</Button><Button type="submit" form="move-container" disabled={save.isPending}>{save.isPending ? 'Saving…' : 'Record move'}</Button></RequiredDialogFooter>
