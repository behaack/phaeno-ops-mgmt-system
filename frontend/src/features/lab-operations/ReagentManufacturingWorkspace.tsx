...
export function ReagentManufacturingWorkspace({ enabled, canOperate }: { enabled: boolean; canOperate: boolean }) {
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={value => { if (!start.isPending) setOpen(value) }}><DialogContent><form className="contents" noValidate onSubmit={form.handleSubmit(value => start.mutate(value))}><DialogHeader><DialogTitle>Start reagent run</DialogTitle><DialogDescription>Choose the reagent. POMS uses its approved workflow and saved inventory unit, assigns a Phaeno lot number, and records the exact procedure revision. Enter the actual quantity produced after the run is complete.</DialogDescription></DialogHeader><div className="space-y-4">
      <PreparationField id="run-reagent" label="Reagent" required error={form.formState.errors.materialDefinitionId?.message}><select id="run-reagent" className={prepSelectClass} {...form.register('materialDefinitionId')}><option value="">Select reagent…</option>{workflows.data?.filter(item => item.status === 'Approved' && item.outputUnit).sort((a, b) => a.materialName.localeCompare(b.materialName)).map(item => <option key={item.id} value={item.materialDefinitionId}>{item.materialName} ({item.outputUnit})</option>)}</select></PreparationField>
      {!workflows.isPending && !workflows.data?.some(item => item.status === 'Approved' && item.outputUnit) ? <p className="text-sm text-muted-foreground">Configure a reagent and approve its workflow in Lab settings before starting a run.</p> : null}
    ...
    </div><RequiredDialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={start.isPending}>Cancel</Button><Button type="submit" disabled={start.isPending || !workflows.data?.some(item => item.status === 'Approved' && item.outputUnit)}>{start.isPending ? 'Starting…' : 'Start run'}</Button></RequiredDialogFooter></form></DialogContent></Dialog>
  </>
}
...
function CompleteDialog({ run, onClose, onSaved }: ActionProps) {
  const form = useForm<z.infer<typeof completeSchema>>({ resolver: zodResolver(completeSchema), defaultValues: { producedQuantity: 0, expirationOrRetestDate: '' } })
