...
export function PreparationResourceField({ field, form, catalog, member, count, defaults = false, correction = false, previous }: {
  ...
  const isMasterMix = field.type === 'material' && Boolean(material?.masterMixWorkflowId)
  const lots = catalog.materialLots.filter(l => materialLotMatches(field, l))
  const mixes = catalog.masterMixes.filter(item => item.workflowId === material?.masterMixWorkflowId && item.workflowRevision === material?.masterMixWorkflowRevision && item.quantityUnit === field.unit)
  ...
  const mix = mixes.find(item => item.id === values[`${exception ? 'shared_' + field.key : prefix}_resource`])
  const quantityUnit = field.type === 'material' ? field.unit?.trim() || lot?.quantityUnit || mix?.quantityUnit : undefined
  ...
  return <fieldset className="min-w-0 rounded-md border p-4"><legend className="px-1 text-sm font-medium">{field.label}{defaults ? ' · Common output values (optional)' : ''}</legend>
    ...
    {field.type === 'equipment' || field.includeTracking || isMasterMix ? <PreparationField id={`${prefix}_resource`} label={isMasterMix ? 'Master-mix preparation' : field.type === 'material' ? 'Lot number' : 'Equipment used'} required={field.type === 'equipment' || field.required || isMasterMix} error={form.formState.errors.values?.[`${prefix}_resource`]?.message}>
      <select id={`${prefix}_resource`} className={prepSelectClass} {...form.register(`values.${prefix}_resource`)}><option value="">Choose…</option>{isMasterMix ? mixes.map(item => <option key={item.id} value={item.id}>{item.barcode} · {item.workflowName} · revision {item.workflowRevision} · {item.remainingQuantity} {item.quantityUnit} remaining · use by {new Date(item.useByUtc).toLocaleString()}</option>) : field.type === 'material' ? lots.map(l => <option key={l.id} value={l.id}>{l.name} · {l.lotNumber} · {l.availableQuantity} {l.quantityUnit}</option>) : catalog.equipment.map(e => <option key={e.id} value={e.id}>{e.name} · {e.assetCode}</option>)}</select>
      {isMasterMix ? <p className="text-xs text-muted-foreground">{mixes.length ? 'Select the single-use preparation shared with this tray. Saving records the actual amount against that mix.' : 'No ready mix of this approved workflow revision is available. Prepare one in Master mixes, or refresh after another operator completes it.'}</p> : field.type === 'material' ? <p className="text-xs text-muted-foreground">{lots.length ? 'Only matching, released lots with available stock are offered. Saving records stock use.' : 'No matching eligible lots are available. Check Materials for product assignment, matching units, QC, expiry and available stock.'}</p> : null}
    </PreparationField> : null}
    {isMasterMix ? <PreparationField id={`${prefix}_mixBarcode`} label="Scan physical master-mix container barcode" required error={form.formState.errors.values?.[`${prefix}_mixBarcode`]?.message}><Input id={`${prefix}_mixBarcode`} autoComplete="off" spellCheck={false} maxLength={50} {...form.register(`values.${prefix}_mixBarcode`)} /></PreparationField> : null}
