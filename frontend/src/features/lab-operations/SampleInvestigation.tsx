  ...
  return <Card className="gap-0 py-0" id="sample-history">
    ...
    <CardContent className="space-y-4 p-4">
      {query.isPending ? <p role="status">Loading sample history…</p> : query.isError ? <EvidenceError error={query.error} /> : data ? <>
        ...
        {(['biologicalMaterialTransfers', 'materials', 'equipment', 'libraries', 'analysisInputs', 'artifacts', 'approvals', 'exceptions', 'custody'] as const).map(section => <EvidenceList key={section} title={evidenceLabel(section)} rows={data.evidence[section] ?? []} />)}
