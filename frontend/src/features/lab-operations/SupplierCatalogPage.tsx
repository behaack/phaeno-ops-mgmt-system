...
export function SupplierCatalogPage({ supplierId }: { supplierId?: string }) {
  ...
  return <div className={supplierId ? 'page-wrap space-y-5 px-4 py-8' : 'space-y-5'}>
    ...
    <Card><CardHeader><CardTitle>{supplier ? 'Products' : 'Suppliers & Products'}</CardTitle><CardDescription>{supplier ? 'Product numbers and descriptions used when preparing kits.' : 'Manage vendors and their reagents, tubes, shipping containers, and other products.'}</CardDescription><CardAction><Button disabled={supplier ? !supplier.isActive : false} onClick={() => supplier ? setProductEditor('new') : setSupplierEditor('new')}>{supplier ? '+ New product' : '+ New supplier'}</Button></CardAction></CardHeader>
      <CardContent className="space-y-4">
        <div className="flex flex-wrap items-end gap-4"><div className="min-w-48 flex-1"><Label htmlFor="supplier-catalog-search">{supplier ? 'Search products' : 'Search suppliers or products'}</Label><Input id="supplier-catalog-search" className="mt-2" value={search} onChange={e => setSearch(e.target.value)} /></div><label className="flex cursor-pointer items-center gap-2 py-2 text-sm"><input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} />Show inactive</label></div>
        {supplier ? products.length ? <ul className="divide-y" aria-label="Products">{products.map(product => <li key={product.id} className="flex flex-wrap items-start justify-between gap-3 py-4"><div className="min-w-0 flex-1 basis-48"><p className="font-medium wrap-anywhere">{product.productNumber}</p><p className="mt-1 wrap-anywhere">{product.description}</p><p className="mt-1 text-sm text-muted-foreground">{product.productTypeName}{product.productTypeIsActive ? '' : ' (type inactive)'} · {product.isActive ? 'Active' : 'Inactive'}</p></div><CatalogActions id={`product-actions-${product.id}`} name={product.productNumber} isActive={product.isActive} onEdit={() => setProductEditor(product)} onStatus={() => setStatusTarget({ kind: 'product', supplier, product })} /></li>)}</ul> : <p className="text-muted-foreground">{supplier.products.length ? 'No products match these filters.' : 'No products yet. Add a product with its number, description and type.'}</p>
...
function SupplierCatalogStatusDialog({ target, onClose }: { target: CatalogStatusTarget; onClose: () => void }) {
  ...
  return <CatalogStatusConfirmation name={name} isActive={record.isActive} description={description} actionId={`${target.kind}-actions-${record.id}`} fallbackId="supplier-catalog-search" onClose={onClose} onSave={async () => {
    if (target.kind === 'supplier') {
      ...
      const updated = await saveSupplierProduct(target.supplier.id, { productNumber: p.productNumber, description: p.description, productTypeId: p.productTypeId, isActive: !p.isActive, version: p.version }, p.id)
