...
import { useSupplierCatalog, saveSupplier, saveSupplierProduct, supplierCatalogKey, transportationKitProductTypeId, type CatalogSupplier, type SupplierProduct } from '#/api/supplier-catalog'
...
export function SupplierCatalogPage({ supplierId }: { supplierId?: string }) {
  ...
  return <div className={supplierId ? 'page-wrap space-y-5 px-4 py-8' : 'space-y-5'}>
    ...
    {supplier ? <>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold wrap-anywhere">{supplier.name}</h1><div className="mt-2 flex gap-2"><Badge variant="secondary">{supplier.isActive ? 'Active' : 'Inactive'}</Badge>{supplier.isInternalProducer ? <Badge variant="outline">Internal producer</Badge> : null}</div></div>{supplier.isInternalProducer ? null : <CatalogActions id={`supplier-actions-${supplier.id}`} name={supplier.name} isActive={supplier.isActive} onEdit={() => setSupplierEditor(supplier)} onStatus={() => setStatusTarget({ kind: 'supplier', supplier })} />}</div>
    ...
    <Card className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-muted/50 p-4"><CardTitle>{supplier ? 'Products' : 'Suppliers & products'}</CardTitle><CardDescription>{supplier?.isInternalProducer ? 'Define each named Phaeno-made reagent or transportation kit here. Configure its approved workflow and shipping specification before production.' : supplier ? 'Product names, descriptions and types for laboratory work and kits.' : 'Manage vendors and their reagents, tubes, shipping containers, and other products.'}</CardDescription><CardAction><Button type="button" disabled={supplier ? !supplier.isActive : false} onClick={() => supplier ? setProductEditor('new') : setSupplierEditor('new')}><Plus data-icon="inline-start" />{supplier ? 'New product' : 'New supplier'}</Button></CardAction>
      ...
      <CardContent className="space-y-4 p-4">
        {supplier ? products.length ? <ul className="divide-y" aria-label="Products">{products.map(product => <li key={product.id} className="flex flex-wrap items-start justify-between gap-3 py-4"><div className="min-w-0 flex-1 basis-48"><p className="font-medium wrap-anywhere">{supplier.isInternalProducer && product.productTypeId === transportationKitProductTypeId ? product.description : product.productNumber}</p><p className="mt-1 wrap-anywhere">{supplier.isInternalProducer && product.productTypeId === transportationKitProductTypeId ? 'SKU ' + product.productNumber : product.description}</p><p className="mt-1 text-sm text-muted-foreground">{product.productTypeName}{product.productTypeIsActive ? '' : ' (type inactive)'} · {product.isActive ? 'Active' : 'Inactive'} · Inventory unit: {product.defaultQuantityUnit ?? 'Needs configuration'} · {product.canExpire ? 'Can expire' : 'Expiration not required'}</p></div><CatalogActions id={`product-actions-${product.id}`} name={supplier.isInternalProducer && product.productTypeId === transportationKitProductTypeId ? product.description : product.productNumber} isActive={product.isActive} onEdit={() => setProductEditor(product)} onStatus={() => setStatusTarget({ kind: 'product', supplier, product })} /></li>)}</ul> : <p className="text-muted-foreground">{supplier.products.length ? 'No products match these filters.' : supplier.isInternalProducer ? 'No Phaeno products yet. Add each reagent or transportation kit before configuring its workflow.' : 'No products yet. Add a product with its name, description and type.'}</p>
          : suppliers.length ? <ul className="divide-y" aria-label="Suppliers">{suppliers.map(item => <li key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-4"><div className="min-w-0"><Link className="font-medium text-primary underline wrap-anywhere" to="/lab-operations/suppliers/$supplierId" params={{ supplierId: item.id }} search={{ section: 'suppliers', supplierTab: 'suppliers', supplierSearch: search || undefined, supplierInactive: showInactive || undefined }}>{item.name}</Link><p className="mt-1 text-sm text-muted-foreground">{item.isInternalProducer ? 'Phaeno-manufactured materials' : `${item.products.filter(p => p.isActive).length} active products`}</p></div><div className="flex items-center gap-3"><Badge variant="secondary">{item.isActive ? 'Active' : 'Inactive'}</Badge>{item.isInternalProducer ? <Badge variant="outline">Internal producer</Badge> : <CatalogActions id={`supplier-actions-${item.id}`} name={item.name} isActive={item.isActive} onEdit={() => setSupplierEditor(item)} onStatus={() => setStatusTarget({ kind: 'supplier', supplier: item })} />}</div></li>)}</ul> : <p className="text-muted-foreground">{data.length ? 'No suppliers match these filters.' : 'No suppliers yet. Add a supplier, then add its products.'}</p>}
      </CardContent>
    </Card>
...
function SupplierCatalogStatusDialog({ target, onClose }: { target: CatalogStatusTarget; onClose: () => void }) {
  ...
  const description = target.kind === 'supplier'
    ...
    : target.supplier.isInternalProducer
      ? record.isActive ? 'New manufacturing runs will be blocked. Existing lots and runs remain in history.' : 'This reagent will be available for new runs once its workflow is approved.'
