...
export function SupplierCatalogPage({ supplierId }: { supplierId?: string }) {
  ...
  return <div className={supplierId ? 'page-wrap space-y-5 px-4 py-8' : 'space-y-5'}>
    ...
    <Card className="gap-0 overflow-hidden py-0"><CardHeader className="border-b bg-muted/50 p-4"><CardTitle>{supplier ? 'Products' : 'Suppliers & products'}</CardTitle><CardDescription>{supplier?.isInternalProducer ? 'Phaeno-made reagents are created through Reagent manufacturing and recorded in Materials.' : supplier ? 'Product names, descriptions and types for laboratory work and kits.' : 'Manage vendors and their reagents, tubes, shipping containers, and other products.'}</CardDescription>{supplier?.isInternalProducer ? null : <CardAction><Button type="button" disabled={supplier ? !supplier.isActive : false} onClick={() => supplier ? setProductEditor('new') : setSupplierEditor('new')}><Plus data-icon="inline-start" />{supplier ? 'New product' : 'New supplier'}</Button></CardAction>}
      ...
      <CardContent className="space-y-4 p-4">
        {supplier ? products.length ? <ul className="divide-y" aria-label="Products">{products.map(product => <li key={product.id} className="flex flex-wrap items-start justify-between gap-3 py-4"><div className="min-w-0 flex-1 basis-48"><p className="font-medium wrap-anywhere">{product.productNumber}</p><p className="mt-1 wrap-anywhere">{product.description}</p><p className="mt-1 text-sm text-muted-foreground">{product.productTypeName}{product.productTypeIsActive ? '' : ' (type inactive)'} · {product.isActive ? 'Active' : 'Inactive'} · Inventory unit: {product.defaultQuantityUnit ?? 'Needs configuration'} · {product.canExpire ? 'Can expire' : 'Expiration not required'}</p></div><CatalogActions id={`product-actions-${product.id}`} name={product.productNumber} isActive={product.isActive} onEdit={() => setProductEditor(product)} onStatus={() => setStatusTarget({ kind: 'product', supplier, product })} /></li>)}</ul> : <p className="text-muted-foreground">{supplier.products.length ? 'No products match these filters.' : supplier.isInternalProducer ? 'Phaeno-made reagent lots appear in Materials.' : 'No products yet. Add a product with its name, description and type.'}</p>
