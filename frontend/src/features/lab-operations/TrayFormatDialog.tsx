...
export function TrayFormatDialog({ format, onClose }: { format?: TrayFormat; onClose: () => void }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open && !save.isPending) onClose() }}><DialogContent><form className="contents" noValidate onSubmit={form.handleSubmit(v => save.mutate(v))}>
    <DialogHeader><DialogTitle>{format ? 'Edit library tray format' : 'New library tray format'}</DialogTitle><DialogDescription>Existing batch layouts remain unchanged when this format is edited or retired.</DialogDescription></DialogHeader>
    <div className="space-y-4">{save.isError ? <p role="alert" className="text-sm text-destructive">{getLabOperationsError(save.error, 'Library tray format could not be saved.')}</p> : null}
      ...
      <div aria-label="Library tray layout preview" className="overflow-x-auto"><div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${layout.columns}, minmax(2.5rem, 1fr))` }}>{trayPositions(layout).map(p => <span key={p} className={`rounded border p-1 text-center text-xs ${layout.unavailable.includes(p) ? 'bg-muted line-through' : 'bg-background'}`}>{p}</span>)}</div></div>
