...
describe('CRM reviewed record snapshots', () => {
  ...
  it('preserves a Contact communication choice and reviewed version through refresh and failed save', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Suppression reason/), { target: { value: 'DirectRequest' } })
    fireEvent.change(screen.getByLabelText(/Decision explanation/), { target: { value: 'Asked to stop sales email' } })
    act(() => client.setQueryData(['crm-contact', contact.id], { ...contact, communicationPreference: 'Permitted', firstName: 'Refreshed', version: 5 }))
    expect(screen.getByLabelText(/Outreach status/)).toHaveProperty('value', 'Suppressed')
    fireEvent.click(screen.getByRole('button', { name: 'Save changes' }))
    await waitFor(() => expect(api.updateCrmContact).toHaveBeenCalledWith(contact.id, expect.objectContaining({ firstName: 'Keep drafted name', communicationPreference: 'Unknown', version: 4,
      outreachDecision: { preference: 'Suppressed', permissionSource: 'DirectRequest', recordedOn: '2026-09-01', suppressionReason: 'DirectRequest', explanation: 'Asked to stop sales email' } })))
