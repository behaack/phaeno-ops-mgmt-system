...
describe('CRM merge dialog', () => {
  ...
  it.each(['Company', 'Contact'] as const)('searches the %s directory beyond the initial choices and excludes the source', async recordLabel => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'Merge reason' }), { target: { value: ' Confirmed duplicate after review. ' } })
    fireEvent.click(screen.getByRole('button', { name: 'Merge records' }))
  ...
  it('preserves a dirty draft after cancelled dismissal, pending work, and a failed merge', async () => {
    ...
    try {
      ...
      fireEvent.change(screen.getByRole('textbox', { name: 'Merge reason' }), { target: { value: 'Reviewed the duplicate records.' } })
      fireEvent.click(screen.getByRole('button', { name: 'Cancel' }))
      expect(confirm).toHaveBeenCalledWith('Discard unsaved merge choices?')
      expect(onClose).not.toHaveBeenCalled()
      update(true)
      expect(screen.getByRole('button', { name: 'Cancel' })).toHaveProperty('disabled', true)
      expect(screen.getByRole('textbox', { name: 'Merge reason' }).closest('fieldset')).toHaveProperty('disabled', true)
      ...
      expect(screen.getByRole('textbox', { name: 'Merge reason' })).toHaveProperty('value', 'Reviewed the duplicate records.')
