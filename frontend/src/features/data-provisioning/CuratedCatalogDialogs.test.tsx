...
describe('Curated catalog dialog draft protection', () => {
  ...
  it.each(cases)('keeps $kind drafts during cancelled dismissal, pending work, and failures', async scenario => {
    ...
    try {
      ...
      fireEvent.change(within(dialog).getByLabelText(scenario.field), { target: { value: 'Reviewed change' } })
      if (scenario.kind === 'create') fireEvent.change(within(dialog).getByLabelText('Description'), { target: { value: 'Reviewed description' } })
      fireEvent.click(within(dialog).getByRole('button', { name: 'Cancel' }))
      expect(confirm).toHaveBeenCalled()
      expect(screen.getByRole('dialog', { name: scenario.title })).toBeTruthy()
      expect(within(dialog).getByLabelText(scenario.field)).toHaveProperty('value', 'Reviewed change')
      ...
      expect(within(dialog).getByLabelText(scenario.field).closest('fieldset')).toHaveProperty('disabled', true)
      ...
      expect(screen.getByLabelText(scenario.field)).toHaveProperty('value', scenario.kind === 'edit' ? dataset.name : '')
