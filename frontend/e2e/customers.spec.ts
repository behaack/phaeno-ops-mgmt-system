...
test("confirms organization deactivation in an accessible dialog", async ({
  page,
}) => {
  ...
  await page.route(apiRequestPattern, async (route) => {
    ...
    if (
      method === "POST" &&
      url.pathname === `/api/organizations/${organizationId}/deactivate`
    ) {
      organization = {
        ...organization,
        isActive: false,
  ...
  await expect(
    page.getByRole("link", { name: "Atlas Research" }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("link", { name: "Open CRM Companies" }),
  ).toBeVisible();
  await expect(
  ...
  await expect(dialog).toContainText(
    "Existing memberships will stop granting access",
  );
  ...
  await expect(page.getByRole("link", { name: "Atlas Research" })).toHaveCount(
    0,
  );
...
test("selects an eligible source request and uses lifecycle dialogs in the organization workspace", async ({
  page,
}) => {
  ...
  await page.route(apiRequestPattern, async (route) => {
    ...
    if (
      method === "GET" &&
      url.pathname === `/api/organizations/${organizationId}`
    ) {
  ...
  const entitlementDialog = page.getByRole("dialog", {
    name: "Add service entitlement",
  });
  const requestSelect = entitlementDialog.getByLabel("Approved source request");
  await expect(
    requestSelect.getByRole("option", { name: /PRQ-SERVICE/ }),
  ).toHaveCount(1);
  await expect(
    requestSelect.getByRole("option", { name: /PRQ-ONBOARDING/ }),
  ).toHaveCount(0);
  ...
  await expect(endDialog.getByRole("alert")).toContainText(
...
function customerOrganization() {
  return {
    ...
    name: "Atlas Research",
    description:
      "Synthetic customer for organization-management browser coverage.",
    kind: "Customer",
