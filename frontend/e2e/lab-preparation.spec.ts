...
test('shared capture and a tube exception retain explicit coverage and accessible fields', async ({ page }, info) => {
  ...
  await page.getByRole('checkbox', { name: 'Record exception', exact: true }).check()
  await page.getByRole('button', { name: /^A2.*TEST-TUBE-2/, exact: false }).click()
...
test('failed tubes are excluded from later evidence and output identity is carried forward', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /^View A2, tube TEST-TUBE-2/ }).click()
  ...
  await page.getByRole('button', { name: /^View A1, tube TEST-TUBE-1/ }).click()
  await page.getByRole('region', { name: 'Tube details for A1' }).getByRole('button', { name: 'Actions', exact: true }).click()
...
test('resource use records the eligible tube coverage', async ({ page }) => {
  const { bodies } = await setup(page, true, { configure: data => { data.stages[0].definition.steps[0].inputMaterials = ['TEST reagent'] } })
  ...
  await page.getByRole('button', { name: 'Record material use', exact: true }).click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  expect(bodies[0]).toMatchObject({ action: 'material', quantity: 2, coveredMemberIds: ['member-0'] })
})
...
test('stage completion identifies the affected stage without an empty required form', async ({ page }) => {
  const { bodies } = await setup(page, false, { configure: data => { data.members.forEach(m => { m.executions[0].blockers = []; m.output = { id: m.id, barcode: 'OUTPUT-'+m.id, quantity: 1, quantityUnit: 'uL', confirmed: true }; m.executions[0].evidence.records = [{ id: 'done', action: 'record', operatorConfirmed: true, resourcesConfirmed: true, reason: null, recordedByUserId: 'operator', stepKey: 'qc', outcome: 'recorded', qcOutcome: 'pass', captures: {}, recordedAtUtc: new Date().toISOString() }] }) } })
  await page.getByRole('button', { name: 'Complete protocol', exact: true }).first().click()
  const dialog = page.getByRole('dialog', { name: 'Complete protocol', exact: true })
  ...
  await expect(dialog.getByText('Step records remain in history.', { exact: false })).toBeVisible()
...
test('resource choices exclude expired lots and overdue equipment but include the due date', async ({ page }) => {
  await setup(page, false, { resourceAvailability: true, configure: data => { data.stages[0].definition.steps[0].inputMaterials = ['TEST reagent']; data.stages[0].definition.steps[0].equipmentTypes = ['Centrifuge'] } })
...
for (const role of ['Operator', 'ScientificReviewer']) {
  test(`${role} cannot record a Supervisor preparation step`, async ({ page }) => {
    ...
    await expect(page.getByRole('region', { name: 'Preparation progress' }).getByRole('button', { name: 'Record step', exact: true })).toHaveCount(0)
...
test('keyboard step cancellation restores focus without saving', async ({ page }) => {
  const { bodies } = await setup(page)
  const trigger = page.getByRole('region', { name: 'Preparation progress' }).getByRole('button', { name: 'Record step', exact: true })
  await trigger.focus()
  ...
  await expect(dialog.getByRole('combobox', { name: 'Performed by', exact: true })).toBeFocused()
  await page.keyboard.press('Tab')
  await expect(dialog.getByRole('combobox', { name: 'Performed time', exact: true })).toBeFocused()
  await page.keyboard.press('Tab')
