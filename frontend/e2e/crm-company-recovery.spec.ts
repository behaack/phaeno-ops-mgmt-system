...
test('Company People and Sales recover failed reads without inviting duplicate work', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Add existing person', exact: true })).toBeDisabled()
  ...
  await expect(page.getByRole('button', { name: 'Add existing person', exact: true })).toBeEnabled()
  ...
  await page.getByRole('button', { name: 'Add existing person', exact: true }).click()
